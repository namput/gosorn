...
/**
 * BlogWritePage – Luxe Tiptap Edition
 * หน้าเขียนบล็อกแบบครบ พร้อมเมตา ฝั่งซ้าย + Editor ฝั่งขวา (Tiptap)
 * - โหมดสว่าง/มืด
 * - ทูลบาร์สวยงาม + ปุ่ม YouTube + รูป (อัปโหลด/URL)
 * - นับอักขระ (เหมาะกับภาษาไทย)
 * - Autosave + onSave/onPublish hooks
 * - รองรับ Link/Image/Youtube ผ่าน Dialog
 */
...
export type BlogDraft = {
  ...
  contentHtml: string; // เก็บ HTML จาก Tiptap
  status: BlogStatus;
  updatedAt: number;
};
...
export type BlogWritePageProps = {
  ...
  /**
   * ถ้าต้องการ override พฤติกรรมการบันทึก/เผยแพร่ ให้ส่งเข้ามาได้
   * ถ้าไม่ส่งมา จะใช้ตัว default ที่เรียก backend (/api/blogs/...)
   */
  onSave?: (draft: BlogDraft) => Promise<void> | void;
  onPublish?: (draft: BlogDraft) => Promise<void> | void;
  onUpload?: (file: File) => Promise<string>; // คืน URL ของไฟล์รูป
};
...
export default function BlogWritePage({
  ...
  const [description, setDescription] = useState(
    initial?.description ?? "สรุปใจความของบทความนี้ไม่เกิน 160 อักขระ"
  );
  const [tags, setTags] = useState<string[]>(
    initial?.tags ?? ["guson", "howto"]
  );
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder:
          "พิมพ์เนื้อหา... ใช้ / เพื่อเรียกคำสั่งเร็ว (เช่น /ภาพ, /ยูทูบ)",
      }),
      Link.configure({
        openOnClick: true,
        autolink: true,
    ...
    content:
      initial?.contentHtml ??
      "<h2>สวัสดี 👋</h2><p>นี่คือ <b>Luxe Tiptap Editor</b> — เขียนสบาย ดูดี และพร้อมเผยแพร่</p>",
    editorProps: {
      attributes: {
        class: "min-h-[62vh] focus:outline-none",
      },
    },
    onCreate: ({ editor }) => {
      ...
      // พยายามส่งคิว outbox หากมี และออนไลน์
      try {
        ...
        if (
          ...
          const box: Array<{ when: number; draft: BlogDraft }> =
            JSON.parse(raw);
          if (box.length) {
            (async () => {
...
function resetEditorAfterPublish() {
  ...
  async function apiSaveDraft(draft: BlogDraft) {
    const res = await fetch(`http://localhost:3000/api/blogs/draft`, {
      method: "POST",
    ...
    // ถ้า backend ส่ง id/version มาก็เก็บไว้ช่วย sync
    if (data && (data.id || data.version)) {
      const key = `${storageKey}:serverMeta`;
      localStorage.setItem(
        key,
        JSON.stringify({ id: data.id, version: data.version, at: Date.now() })
      );
    }
  }
  ...
  const manualSave = async () => {
    ...
    const draft = currentDraft();
    try {
      // บันทึกโลคัลเสมอ (กันเน็ตล่ม)
      localStorage.setItem(storageKey, JSON.stringify(draft));
      setLastSaved(draft.updatedAt);

      // ถ้าออฟไลน์ ให้เก็บเข้าคิว outbox แล้วแจ้งเตือน
      if (
        typeof navigator !== "undefined" &&
        navigator &&
        navigator.onLine === false
        ...
        setToast({
          kind: "warn",
          text: "ออฟไลน์: เก็บลงคิว ส่งให้อัตโนมัติเมื่อออนไลน์",
        });
        return;
      }
      ...
      // เก็บเข้าคิวกรณีเซิร์ฟเวอร์ตอบไม่ได้
      try {
        const key = `${storageKey}:outbox`;
        const box = JSON.parse(localStorage.getItem(key) || "[]");
        box.push({ when: Date.now(), draft });
      ...
      setToast({
        kind: "error",
        text: "บันทึกไม่สำเร็จ: จะลองส่งใหม่อัตโนมัติ",
      });
    } finally {
  ...
  const handlePublish = async () => {
    ...
    if (!title || title.trim().length < 5) {
      setWarning("ตั้งชื่อบทความอย่างน้อย 5 อักขระ");
      setIsPublishing(false);
      return;
    }
    if (!slug) {
      setWarning("Slug ว่างไม่ได้");
      setIsPublishing(false);
      return;
    }
    if ((editor?.getText() ?? "").trim().length < 100) {
      setWarning("เนื้อหาควรยาวอย่างน้อย ~100 อักขระ");
      setIsPublishing(false);
      return;
    }
    if (description.length > 160) {
      setWarning("Meta description ไม่ควรเกิน 160 อักขระ");
      setIsPublishing(false);
      return;
    }
    setWarning(null);
  ...
  const descPct = Math.min(
    100,
    Math.max(0, Math.round((description.length / 160) * 100))
  );
      ...
      <div className="sticky top-0 z-50 border-b border-white/20 bg-white/60 backdrop-blur-xl dark:border-white/10 dark:bg-neutral-950/50">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <div className="grid h-9 w-9 place-items-center rounded-2xl bg-gradient-to-tr from-indigo-600 to-violet-600 text-white shadow-sm">
              ✍️
            </div>
            <div>
              <div className="text-sm font-semibold leading-5">
                Guson • Luxe Tiptap Editor
              </div>
              <div className="text-xs opacity-70">
                {saving
                  ? "กำลังบันทึก…"
                  : lastSaved
                  ? `บันทึกล่าสุด: ${new Date(lastSaved).toLocaleString()}`
                  : "ยังไม่เคยบันทึก"}
              </div>
            </div>
          </div>
        ...
        <aside className="order-2 space-y-4 lg:order-1">
          ...
          <div className="rounded-3xl border border-neutral-200/70 bg-white/70 p-4 shadow-lg backdrop-blur dark:border-white/10 dark:bg-neutral-900/60">
            <div className="flex items-center gap-2 text-sm font-medium">
              <PencilLine className="h-4 w-4" /> ชื่อบทความ
            </div>
            <input
              ...
              placeholder="เช่น 10 เทคนิคการเขียนข่าวให้น่าอ่าน"
            />
          ...
          <div className="rounded-3xl border border-neutral-200/70 bg-white/70 p-4 shadow-lg backdrop-blur dark:border-white/10 dark:bg-neutral-900/60">
            <div className="mb-1 flex items-center justify-between">
              ...
              <span
                className={`text-xs ${
                  descLeft < 0 ? "text-rose-500" : "opacity-60"
                }`}
              >
            ...
            <textarea
              ...
              placeholder="สรุปเนื้อหาภายใน 160 อักขระ"
            />
            <div className="mt-3 flex items-center justify-between">
              <div className="relative h-10 w-10">
                ...
                />
                <div className="absolute inset-[4px] rounded-full bg-white/80 dark:bg-neutral-900/80 grid place-items-center text-[10px]">
                  {descPct}%
                </div>
              </div>
          ...
          <div className="rounded-3xl border border-neutral-200/70 bg-white/70 p-4 shadow-lg backdrop-blur dark:border-white/10 dark:bg-neutral-900/60">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {tags.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center gap-2 rounded-full bg-indigo-50/80 px-3 py-1 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200 dark:bg-indigo-900/40 dark:text-indigo-100 dark:ring-indigo-800"
                >
                  #{t}
                  <button
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                onClick={() => {
                  ...
                  if (!u) return;
                  if (/^http:\/\//i.test(u))
                    setWarning("http:// จะไม่แสดงบน https (Mixed Content)");
          ...
          <div className="mt-4">
            {warning ? (
              <div className="flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800 shadow-sm">
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs opacity-70">
                    โปรดตรวจสอบการเชื่อมต่อ หรือรีเฟรชหน้าใหม่อีกครั้ง
                  </div>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 p-2 text-xs text-blue-700 shadow-sm">
                ...
                <span>
                  กด Ctrl/Cmd + S เพื่อบันทึก • ลิงก์จะเปิดแท็บใหม่อัตโนมัติ
                </span>
              </div>
            )}
          </div>
          ...
          <div className="sticky top-[68px] z-40 mb-2 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-neutral-200/70 bg-white/70 px-2 py-1 shadow-lg backdrop-blur dark:border-white/10 dark:bg-neutral-900/60">
            <div className="flex flex-wrap items-center gap-1">
              ...
              <IconWrap
                ...
                tooltip="แทรกรูป (อัปโหลด/URL)"
              >
                <ImageIcon className="h-4 w-4" />
              </IconWrap>
              <IconWrap
                onClick={() => setShowYTDialog(true)}
                tooltip="แทรก YouTube"
              >
...
function toYoutubeUrl(input: string) {
  // รับ ID หรือ URL → คืน URL embed-friendly
  try {
...
function IconWrap({
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative rounded-xl p-2 hover:bg-neutral-100/80 dark:hover:bg-neutral-800/70`}
    >
      {children}
...
function Dialog({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg overflow-hidden rounded-2xl border border-white/10 bg-white/95 shadow-2xl backdrop-blur dark:bg-neutral-900/95"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-white/20 px-4 py-2 text-sm font-medium dark:border-white/10">
          {title}
        </div>
